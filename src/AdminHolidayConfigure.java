...
import controllerClasses.HolidayDateControl;
...
public class AdminHolidayConfigure {
	...
	public void displayHolidayMain() throws IOException, ParseException
	{
		int choice;
		Scanner sc = new Scanner(System.in);

		HolidayDateControl HDC = new HolidayDateControl();
		System.out.println("#############################################");
	...
	public void displayCreate() throws IOException, ParseException
	{
		Scanner sc=new Scanner(System.in);
		Date day;
		HolidayDate hD;
	do{
		...
		day=ValidationControl.validateDate(sc.nextLine());
		hD=new HolidayDate();
		hD.setHolidayDate(day);
		
		}while(day==null);
		HolidayDateControl.createHoliday(hD);
		displayHolidayMain();
	
	}
	
	
	public void displayUpdateSelection(ArrayList<HolidayDate> holidayList) throws ParseException, IOException
	{
		if(!holidayList.isEmpty())
		{
			int choice=0;
			...
			do
			{
				...
				System.out.println("");
				choice=ValidationControl.validateAndReturnIntegerValue(sc.nextLine());
				}while(choice==2||(choice>holidayList.size()));
	...
	public void displayUpdatePage(ArrayList<ObjectContainer> oList, int choice) throws ParseException, IOException{
		int id = 0;
		String date="";
		...
		do
		{
			...
			if(selected==1){
				do{
					...
					Date nHD=ValidationControl.validateDate(s);
					valid=HolidayDateControl.validateDuplicatedHoliday(s);
					if(valid)
					{
						HolidayDate hD=new HolidayDate();
						hD.setHolidayDate(nHD);
						hD.setHolidayId(id);
						HolidayDateControl.updateHoliday(id,hD);
					}
					else
					{
			...
			if(selected==2){
				...
				do{
					...
					if(confirm==1){
						HolidayDateControl.removeHolidayDate(id);
					}
					else if(confirm==2)
					{
						displayUpdatePage(oList,choice);
