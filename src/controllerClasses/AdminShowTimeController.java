...
/**
 * Class that communicate with ShowTimeDataControl and deal with mainly the
 * formating the UI that deals with CRUD of time slots
 * 
 * @author Chang En Kai
 *
 */
public class AdminShowTimeController extends AdminSchedulerController {
	...
	/**
	 * Check for timeslot clash when admin insert new time slot for movie. This
	 * method is used during update of timeslot.
	 * 
	 * @param movieLen
	 *            Movie length
	 * @param cinemaId
	 ...
	 */
	public static boolean checkTimeSlotClashforUpdate(int movieLen,
			int cinemaId, String inputTime, String time2, Calendar cal)
			throws IOException, ParseException {
		boolean valid = true;
		String sED = TimeDateControl.minutesPlusTime(movieLen, inputTime);

		String[] seDarr = sED.split("-");
		ArrayList<String> allocatedShowTimeC = cinemaallocatedTime(cinemaId,
				cal);
		...
		for (int i = 0; i < allocatedShowTimeC.size(); i++) {
			...
			Date startTimeExist = new SimpleDateFormat("HHmm")
					.parse(timeArr[0]);
			Date endTimeExist = new SimpleDateFormat("HHmm").parse(timeArr[1]);

			Date startTimeUpdate = new SimpleDateFormat("HHmm")
					.parse(timeArr2[0]);
			Date endTimeUpdate = new SimpleDateFormat("HHmm")
					.parse(timeArr2[1]);

			if (startTimeUpdate.after(startTimeExist)
					&& startTimeUpdate.before(endTimeExist)
					|| startTimeUpdate.equals(startTimeExist)) {
				valid = true;
				break;
			}

			else if (startTime.before(endTimeExist)
					&& startTime.after(startTimeExist)
					|| startTime.equals(startTimeExist)) {
				System.out.println("Time clashed! Please try again!");
				valid = false;
				break;
			}

			else if (startTimeExist.after(startTime)
					&& startTimeExist.before(endTime)) {
				System.out.println("Time clashed! Please try again!");
				valid = false;
				break;
			}
	...
	/**
	 * Check for timeslot clash when admin insert new time slot for movie. This
	 * method is used during creation of timeslot for the first time.
	 * 
	 * @param cinemaId
	 * @param inputTime
	 * @param cal
	...
	public static boolean checkTimeSlotClash(int cinemaId, String inputTime,
			Calendar cal, int movieLen) throws IOException, ParseException {
		boolean valid = true;
		String sED = TimeDateControl.minutesPlusTime(movieLen, inputTime);
		String[] seDarr = sED.split("-");
		ArrayList<String> allocatedShowTimeC = cinemaallocatedTime(cinemaId,
		...
		for (int i = 0; i < allocatedShowTimeC.size(); i++) {
			...
			Date endTimeTimeExist = new SimpleDateFormat("HHmm")
					.parse(timeArr[1]);

			if (startTime.before(endTimeTimeExist)
					&& startTime.after(startTimeExist)
					|| startTime.equals(startTimeExist)) {
				System.out.println("Time clashed! Please try again!");
				valid = false;
	 ...
	 *            cinemaId
	 * @param tmp
	 *            Calendar object that contain dates to check for
	 * @return
	 * @throws IOException
	 * @throws ParseException
	 */
	public static ArrayList<String> cinemaallocatedTime(int cinemaId,
			Calendar tmp) throws IOException, ParseException {
		ArrayList<ShowTime> stC = ShowTimeDataControl
				.readShowTimesBasedOnCinemaIdAndNowShowing(cinemaId, tmp);
		// ArrayList<ObjectContainer> allocatedTime=new
	 ...
	 * 
	 * @param operation
	 *            Can be add, update or create, use to differentiate which
	 *            operation is using this function
	 * @param cinpleId
	 *            cineplex ID
	 * @param sch
	 *            Movie schedule
	 ...
	 * @throws ParseException
	 */
	public static void timeSlotHandler(String operation, int cinpleId,
			MovieSchedule sch, int movieId, int movieLen, int movieType,
			int runDate, Movie movie) throws IOException, ParseException {
		...
		if (cinpleId == 0) {
			...
			do {
				System.out
						.println("Please choose which cineplex to create time slot for selected movie(input -1 to exit).");
				...
				if (tmps.equals("-1")) {
					if (operation.equals("c")) {
						...
						AdminSchedulerUpdateUi.displayEditExistingTimeSlot(sch,
								movieId, cinpleId, movie);
				}
				cineplexId = ValidationControl
						.validateAndReturnIntegerValue(tmps);
			...
			for (int j = 0; j < cinemaList.size(); j++) {
				if (cinemaList.get(j).getCinemaType() == platOrNot) {
					System.out.print(count + ":"
							+ cinemaList.get(j).getCinemaHallName() + "	");
					pair.add(MiscControl.idPairerWithCinema(j, cinemaList
							.get(j).getCinemaId(), cinemaList.get(j)
							.getCinemaHallName(), cinemaList.get(j)
							.getCinemaType(), cinemaList.get(j).getSeats()));
					count++;
				}
			}
			System.out.println();
		...
		for (int d = 1; d <= runDate; d++) {
			...
			do {
				if (sch.getPreviewStatus() == 1) {
					...
					System.out.println("Please enter preview show time for "
							+ tmpDate);
			...
			while (counter < num) {
				do {
					...
					System.out.println("This " + cinemaName
							+ " already have these time slot allocated on "
							+ tmpDate);
					allocatedShowTime = AdminShowTimeController
					...
					String s = sc.nextLine();
					if (s.equals("-1")) {
						timeSlotHandler(operation, cinpleId, sch, movieId,
								movieLen, movieType, runDate, movie);
					}
					...
					if (valid2 && time != -2) {
						showTimeValue = TimeDateControl.minutesPlusTime(
								movieLen, s);
