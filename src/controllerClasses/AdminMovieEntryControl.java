...
/**
 * Class that communicate with MovieData Control to deal with CRUD of movie
 * entries.
 * 
 * @author Chang En Kai
 *
...
public class AdminMovieEntryControl {
	 ...
	 * 
	 * @param m
	 *            Movie details that contain cast to be removed
	 * @return
	 */
	public static String removeCast(Movie m) {
		...
		System.out.println((castArr.length + 1)
				+ ".Return back to previous page");
		do {
			System.out
					.println("Select number beside cast member to remove them from cast list.");
	...
	public static String addCast(Movie m) {
		...
		do {
			System.out
					.println("Enter new cast member(input -1 to return to previous page).");
			add = sc.nextLine();
	...
	public static String updateCast(Movie m) {
		...
		do {
			System.out
					.println("Select number beside cast member to edit selected cast.");
			choice = ValidationControl.validateAndReturnIntegerValue(sc
					.nextLine());
		} while (choice <= 0 || choice > (castArr.length + 1));
	...
	/**
	 * Check to see if movie can remove or not, if transcation of movie exist,
	 * it will stop user from removing the movie
	 * 
	 * @param temp
	 *            Movie details
	 * @throws IOException
	 ...
	 */
	public static void removeMovieCheckHandler(Movie temp) throws IOException,
			ParseException {
	...
	public static void handleMovieEdit(Movie temp) throws IOException,
			ParseException {
		int choice2 = 0;
		Scanner sc = new Scanner(System.in);
		do {
			...
			choice2 = ValidationControl.validateAndReturnIntegerValue(sc
					.nextLine());
		...
		if (choice2 == 1) {
			...
			do {
				System.out
						.println("Please enter edited movie name(input -1 to return):");
				check = sc.nextLine();
			...
			do {
				System.out
						.println("Please enter age rating(input -1 to return):");
				check = sc.nextLine();
				valid = ValidationControl.validateEmptyString(check);
				if (check.equals("-1")) {
					handleMovieEdit(temp);
			...
			do {
				System.out
						.println("Please enter director name(input -1 to return):");
				check = sc.nextLine();
				valid = ValidationControl.validateEmptyString(check);
				if (check.equals("-1")) {
					handleMovieEdit(temp);
			...
			do {
				...
				castChoice = ValidationControl.validateAndReturnIntegerValue(sc
						.nextLine());
			} while (castChoice <= 0);
			...
			do {
				System.out
						.println("Enter new synopsis(input -1 to return to previous page):");
				check = sc.nextLine();
				valid = ValidationControl.validateEmptyString(check);
				if (check.equals("-1")) {
					handleMovieEdit(temp);
