...
/**
 * Class that deals with actual CRUD of transaction. Transactions happen when a
 * customer buys movie tickets/
 * 
 * @author Chang En Kai
 *
 */
public class TransactionDataControl extends DataControl {
	...
	public static ArrayList<Transaction> readTranscation() throws IOException {
		...
		for (int i = 0; i < stringArray.size(); i++) {
			...
			Transaction u = new Transaction(transcationId, email, mobilePhone,
					custName, age, cineplexI, cinemaId, movie, numSeat, seats,
					price, showTimeId);
			// add to list
			alr.add(u);
		}
