...
public class MovieDataControl extends DataControl {
	...
	public static ArrayList<Movie> readMovie() throws IOException {
		...
		for (int i = 0; i < stringArray.size(); i++) {
			...
			Movie m = new Movie(movieId, movieName, movieType, ageRating,
					director, synopsis, cast, overallRating, ticketSales, time,
					block);
			// add to list
			alr.add(m);
	...
	public static Movie readMovieBasedOnId(int mId) throws IOException {
		...
		for (int i = 0; i < stringArray.size(); i++) {
			...
			if (movieId == mId) {
				Movie m = new Movie(movieId, movieName, movieType, ageRating,
						director, synopsis, cast, overallRating, ticketSales,
						time, block);
				return m;
			}
