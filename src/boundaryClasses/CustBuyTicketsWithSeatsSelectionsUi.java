...
public class CustBuyTicketsWithSeatsSelectionsUi extends
	 ...
	 */
	public static void displayCustomerInfo(int showTimeId, Movie m,
			int listingId,String lType, int plat,int cineplexId) throws IOException, ParseException {
		ShowTime sTList = ShowTimeDataControl
				.readShowTimesBasedOnShowTimeId(showTimeId);
		ObjectContainer cust = CustBuyTicketControl.customerManagement(sTList,
				m, listingId);
		displaySeat(showTimeId, m, cust, lType, plat, listingId, cineplexId);
	}
	 ...
	 *            Movie object
	 * @param cust
	 *            A object container that contains transaction, movie, movie
	 *            show time object
	 * @throws IOException
	 * @throws ParseException
	 */
	public static void displaySeat(int showTimeId, Movie m, ObjectContainer cust,String lType, int plat, int listingId, int cineplexId)
			throws IOException, ParseException {
		ShowTime sTList = ShowTimeDataControl
				.readShowTimesBasedOnShowTimeId(showTimeId);
		Scanner sc = new Scanner(System.in);
		System.out.println(m.getMovieName());
		ArrayList<ObjectContainer> selectedSeats = CustSeatsControl
				.manageSeats(sTList, showTimeId);
		System.out.println();
		if (cust.isOld()) {
		...
		do {
			...
			System.out.println("2.Rechoose time slot");
		...
		if (choice == 1) 
		{
			...
			displayPurchaseSeat(showTimeId, seatList, selectedSeats, cust,
					sTList, m, lType, plat, listingId, cineplexId);
		}
		else if(choice==2)
		{
	 ...
	 * @throws ParseException
	 */
	public static void displayPurchaseSeat(int showTimeId,ArrayList<Seats> seatList,ArrayList<ObjectContainer> selectedSeats, ObjectContainer cust,
			ShowTime sTlist, Movie m,String lType, int plat, int listingId, int cineplexId) throws IOException, ParseException {
		...
		do {
			...
			if (valid) {
				...
				SeatsDataControl.reflectSeatSelections(seatList, seatsToValidate);
				...
				System.out.println("\nInvalid seat combination, there might be a gap between your choosen seats!");
		...
		do{
			...
			choice = ValidationControl.validateYesNoAndReturnIntegerValue(sTemp);
		}while(choice<=0);
