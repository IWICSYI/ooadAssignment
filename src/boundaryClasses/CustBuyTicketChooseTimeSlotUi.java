...
public class CustBuyTicketChooseTimeSlotUi extends CustMovieDetailUi {
	...
	public void displayBuyTicket(String lType,int movieId,int hit,int cineplexId, Movie movieDetails, int listingId,int plat) throws IOException, ParseException
	{
		...
		{
			displayTicketForDay(pair.get(choice-1),movieDetails,listingId, lType, plat, cineplexId);
	 ...
	 * @throws ParseException
	 */
	public void displayTicketForDay(ObjectContainer pair,Movie m,int listingId,String lType,int plat,int cineplexID) throws IOException, ParseException
	{
		
		ArrayList<ObjectContainer> pairingIdWithSlot=new ArrayList<ObjectContainer>();
		for(int i=0;i<pair.getShowTimeList().size();i++)
		{

			ObjectContainer o=new ObjectContainer();
			o.setI(i+1);
			o.setShowTimeId(pair.getShowTimeList().get(i).getShowTimeId());
			o.setTimeValue(pair.getShowTimeList().get(i).getShowTimeValue());
			o.setsT(pair.getShowTimeList().get(i));
